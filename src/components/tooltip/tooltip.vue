<template>
  ...
  >
    <div class="co-tooltip__reference" ref="reference" @click="onClick">
      <slot></slot>
    </div>
    <transition :name="transition" v-if="animation">
      <div
        v-show="visible || always"
...
export default {
  ...
  mounted() {
    if (this.trigger === "focus") {
      ...
      this.focusOff = elm.addEventListener("focus", this.onFocus, false);
      this.blurOff = elm.addEventListener("blur", this.onFocus, false);
    }
  },
  beforeDestroy() {
    if (this.timeoutID) {
